	...
	if (button.value == "Edit") {
		...
		for (var x = startNum; x <= endNum; x++) {
			...
			if (x === 7) {
				...
				var oldData = row.childNodes[x].innerHTML;
				row.childNodes[x].innerHTML = "<select>" + '<option value="Expense">Expense</option>' + '<option value="Gas">Gas</option>' + "</select>";
				row.childNodes[x].childNodes[0].childNodes.forEach(function(option) {
					if (option.value == oldData) {
						option.selected = true;
					}
				...
				var oldData = row.childNodes[x].innerHTML;
				row.childNodes[x].innerHTML =
					"<select>" + '<option value="Log On">Log On</option>' + '<option value="Log Off">Log Off</option>' + "</select>";
				row.childNodes[x].childNodes[0].childNodes.forEach(function(option) {
					if (option.value == oldData) {
						option.selected = true;
					}
